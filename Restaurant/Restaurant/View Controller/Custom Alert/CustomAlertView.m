...
#define ALERT_TITLE_FONT ((iDIOM_ALERT==iPAD_ALERT) ? [UIFont fontWithName:_roboto size:22] : [UIFont fontWithName:_roboto size:20])
#define CONTENT_FONT ((iDIOM_ALERT==iPAD_ALERT) ? [UIFont fontWithName:_roboto size:18] : [UIFont fontWithName:_roboto size:15])
#define BTN_TITLE_FONT ((iDIOM_ALERT==iPAD_ALERT) ? [UIFont fontWithName:_roboto size:16] : [UIFont fontWithName:_roboto size:14])
         ...
         showsImage:(BOOL)_showImage
{
    if (self = [super init])
    {
        ...
        UIFont *_font = [UIFont fontWithName:_roboto_light size:16];
        ...
        self.rightBtn.titleLabel.font = [UIFont fontWithName:_roboto size:18];
        self.leftBtn.titleLabel.font = [UIFont fontWithName:_roboto size:18];
        ...
        [self.rightBtn setBackgroundColor:[UIColor colorWithRed:138.0/255.0 green:211.0/255.0 blue:153.0/255.0 alpha:1]];
        [self.leftBtn setBackgroundColor:[UIColor colorWithRed:138.0/255.0 green:211.0/255.0 blue:153.0/255.0 alpha:1]];
...
- (id)initWithForgotPassword:(NSString *)title
{
    if (self = [super init])
    {
        ...
        textEmailAdd.frame = CGRectMake(10, 60, mainBackgroundView.frame.size.width-20, 35);
        ...
        textEmailAdd.layer.borderColor = [kGrayTextColor CGColor];
        textEmailAdd.layer.borderWidth = 0.5f;
        textEmailAdd.layer.cornerRadius = 5;
        [mainBackgroundView addSubview:textEmailAdd];

        UIView *paddingPassword = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 7, 32)];

        if(isEnglish)
        {
            textEmailAdd.leftViewMode = UITextFieldViewModeAlways;
            textEmailAdd.leftView = paddingPassword;
        }
        else
        {
            textEmailAdd.rightViewMode = UITextFieldViewModeAlways;
            textEmailAdd.rightView = paddingPassword;
        }
        
        UILabel *textSeparator = [[UILabel alloc]initWithFrame:CGRectMake(10, textEmailAdd.frame.origin.y+40, mainBackgroundView.frame.size.width-20, 1)];
        textSeparator.backgroundColor = kSeparaterColor;
        //[mainBackgroundView addSubview:textSeparator];
        
        yPos = labelDescription.frame.origin.y + textEmailAdd.frame.size.height + descriptionSeperatorSpace;
        ...
        [self.rightBtn setTitle:LocalizedString(@"send_mail") forState:UIControlStateNormal];
        [self.leftBtn setTitle:LocalizedString(@"cancel") forState:UIControlStateNormal];
